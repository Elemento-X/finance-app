...
function shouldGenerateToday(
  ...
  switch (recurring.frequency) {
    ...
    case 'yearly': {
      ...
      return (
        today.getUTCMonth() + 1 === monthOfYear &&
        today.getUTCDate() === targetDayYearly
      )
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.cron.info(
      `Starting recurring transactions generation for ${todayStr}`,
    )
    ...
    logger.cron.info(
      `Found ${recurringList.length} active recurring transactions`,
    )
    ...
    for (const recurring of recurringList as RecurringTransactionRow[]) {
      ...
      if (insertError) {
        logger.cron.error(
          `Error inserting transaction for recurring ${recurring.id}:`,
          insertError,
        )
        errors++
        continue
      ...
      if (updateError) {
        logger.cron.error(
          `Error updating last_generated_date for ${recurring.id}:`,
          updateError,
        )
      ...
      logger.cron.info(
        `Generated transaction for recurring ${recurring.id} (${recurring.category})`,
      )
      generated++
    }
