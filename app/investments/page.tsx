...
export default function InvestmentsPage() {
  ...
  const { loadAssets, refreshMarketData, isLoading, lastUpdate } = useInvestmentsStore()
          ...
          <div>
            {lastUpdate && (
              <p className="text-sm text-muted-foreground mt-1">
                {t("investments.lastUpdate")}: {new Date(lastUpdate).toLocaleTimeString()}
              </p>
            )}
          </div>
          ...
          <TabsContent value="portfolio" className="space-y-6">
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold">{t("investments.assetsByClass")}</h2>
              ...
              <Tabs value={activeAssetClass} onValueChange={(v) => setActiveAssetClass(v as AssetClass)}>
                ...
                <div className="mt-6">
                  <AssetsList assetClass={activeAssetClass} isLoading={isLoading} />
                </div>
              </Tabs>
            </Card>
          </TabsContent>
