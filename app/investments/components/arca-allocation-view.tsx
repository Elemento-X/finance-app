...
export function ARCAAllocationView({ isLoading }: ARCAAllocationViewProps) {
  ...
  const getArcaLabel = (category: ARCACategory) => {
    return t(ARCA_CATEGORY_KEYS[category] as keyof typeof import("@/lib/i18n").translations.en)
  }
  ...
  if (!portfolioSummary) {
    return (
      <Card className="p-6">
        <p className="text-center text-muted-foreground">{t("arca.noInvestments")}</p>
      </Card>
    )
  }

  const arcaAllocation = investmentsCalculationsService.calculateARCAAllocation(portfolioSummary)

  const chartData = Object.entries(arcaAllocation.current).map(([category, percentage]) => ({
    name: getArcaLabel(category as ARCACategory),
    value: percentage,
    fill: ARCA_COLORS[category as ARCACategory],
  }))

  const targetChartData = Object.entries(arcaAllocation.target).map(([category, percentage]) => ({
    name: getArcaLabel(category as ARCACategory),
    value: percentage,
    fill: ARCA_COLORS[category as ARCACategory],
  }))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Current Allocation */}
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">{t("arca.currentAllocation")}</h3>
          <div className="h-64">
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold mb-4">{t("arca.allocationDetails")}</h3>
        <div className="space-y-6">
          {Object.entries(arcaAllocation.current).map(([category, currentPercentage]) => {
            ...
            const StatusIcon = status === "ideal" ? CheckCircle2 : difference < 0 ? TrendingDown : TrendingUp

            const statusColor =
              status === "ideal" ? "text-income" : status === "below" ? "text-warning" : "text-expense"

            const statusText = status === "ideal"
              ? t("arca.onTarget")
              : status === "below"
                ? t("arca.below", { percent: Math.abs(difference).toFixed(1) })
                : t("arca.above", { percent: Math.abs(difference).toFixed(1) })

            return (
              <div key={category} className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h4 className="font-medium">{getArcaLabel(categoryKey)}</h4>
                    <Badge variant={status === "ideal" ? "default" : "outline"} className={statusColor}>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>{t("arca.current")}: {currentPercentage.toFixed(2)}%</span>
                    <span>{t("arca.target")}: {target}%</span>
                  </div>
                </div>
              </div>
            )
      ...
      {/* Recommendations */}
      <Card className="p-6">
        <h3 className="text-lg font-semibold mb-4">{t("arca.recommendations")}</h3>
        <div className="space-y-3">
          {Object.entries(arcaAllocation.status).map(([category, status]) => {
            ...
            const categoryKey = category as ARCACategory
            const difference = arcaAllocation.difference[categoryKey]
            const currentValue = portfolioSummary.currentValue * (arcaAllocation.current[categoryKey] / 100)
            ...
            return (
              <Alert key={category}>
                {status === "below" ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                <AlertDescription>
