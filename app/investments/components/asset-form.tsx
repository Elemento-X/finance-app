...
export function AssetForm({
  ...
  const [formData, setFormData] = useState({
    ...
    averagePrice: existingAsset
      ? formatCurrencyInput(
          (existingAsset.averagePrice * 100).toString(),
          profile.currency,
        )
      : '',
    purchaseDate:
      existingAsset?.purchaseDate || new Date().toISOString().split('T')[0],
  })
  ...
  const parseAveragePrice = (value: string): number => {
    ...
    return (
      Number.parseFloat(numericValue.replace(/\./g, '').replace(',', '.')) || 0
    )
  }
  ...
  const assetClassLabel = t(
    ASSET_CLASS_TRANSLATION_KEYS[
      assetClass
    ] as keyof typeof import('@/lib/i18n').translations.en,
  )

  const currencySymbol =
    profile.currency === 'BRL' ? 'R$' : profile.currency === 'EUR' ? '€' : '$'

  const totalInvested =
    Number.parseFloat(formData.quantity || '0') *
    parseAveragePrice(formData.averagePrice)
