...
export function exportToPDF(options: ExportOptions): void {
  ...
  const summaryItems = [
    { label: locale === "pt-BR" ? "Receitas" : "Income", value: totals.income, color: [16, 185, 129] },
    { label: locale === "pt-BR" ? "Despesas" : "Expenses", value: totals.expense, color: [239, 68, 68] },
    { label: locale === "pt-BR" ? "Investimentos" : "Investments", value: totals.investment, color: [59, 130, 246] },
    { label: locale === "pt-BR" ? "Saldo" : "Balance", value: balance, color: balance >= 0 ? [16, 185, 129] : [239, 68, 68] },
  ]
  ...
  if (sortedCategories.length > 0) {
    doc.setFont("helvetica", "bold")
    doc.setFontSize(14)
    doc.text(locale === "pt-BR" ? "Despesas por Categoria" : "Expenses by Category", 20, yPosition)
    yPosition += 10
    ...
    sortedCategories.forEach(([category, amount]) => {
      const percentage = totals.expense > 0 ? ((amount / totals.expense) * 100).toFixed(1) : "0"
      doc.text(`${category}`, 20, yPosition)
      doc.text(`${formatCurrency(amount, currency, locale)} (${percentage}%)`, 100, yPosition)
      yPosition += 6
  ...
  if (recentTransactions.length > 0) {
    ...
    recentTransactions.forEach((t) => {
      ...
      doc.text(getCategoryName(t.category, categories).substring(0, 20), 80, yPosition)
