...
export function PeriodFilter() {
  const { filterPeriod, setFilterPeriod, profile, isHydrated } = useFinanceStore()
  const t = (key: string) => translations[profile.language || "en"][key as keyof typeof translations.en] || key
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
      <Select
        ...
        onValueChange={(value) =>
          setFilterPeriod({ ...filterPeriod, type: value as "day" | "week" | "month" | "year" })
        }
      >
      ...
      <div className="flex items-center gap-2 flex-1">
        <Button size="icon" variant="outline" onClick={() => navigatePeriod("prev")}>
          <ChevronLeft className="size-4" />
        </Button>

        <div className="flex-1 text-center font-medium capitalize">{getDateLabel()}</div>

        <Button size="icon" variant="outline" onClick={() => navigatePeriod("next")}>
